...
class Stage(nn.Module):
    ...
    def __init__(self, block, inplanes, planes, group_width, blocks, stride=1, dilate=False, cheap_ratio=0.5):
        ...
        raw_planes = int(planes * (1 - cheap_ratio) / group_width) * group_width # 1 计算正常block提取的通道数
        cheap_planes = planes - raw_planes # 2 计算需要廉价生成的通道数
    ...
    def forward(self, input):
        x0 = self.base(input) # cin cout
        
        m_list = [x0]
        e = x0[:, :self.raw_planes]  # cout 部分通道
        for l in self.layers:
            e = l(e)
            m_list.append(e) # cout blocks 输出 
        m = torch.cat(m_list,1)
        m = self.merge(m) # planes+raw_planes*(blocks-2) cheap_planes
...
def g_ghost_regnetx_002(**kwargs):
    return GGhostRegNet(Bottleneck, [1, 1, 4, 7], [24, 56, 152, 368], group_width=8, **kwargs) #通道卷积的通道数
